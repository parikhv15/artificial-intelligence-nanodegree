...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        return all([self.parent_layer.is_mutex(actA, actB) for actA in self.parents[literalA] for actB in self.parents[literalB]]) and all([self.parent_layer.is_mutex(actB, actA) for actA in self.parents[literalA] for actB in self.parents[literalB]])

    def _negation(self, literalA, literalB):
        """ Return True if two literals are negations of each other """
        return literalA.__invert__() == literalB
...
class PlanningGraph:
    ...
    def _generate_literals_cost(self):
        literal_costs = dict()
        for index, layer in enumerate(self.literal_layers):
            for node in layer:
                if node not in literal_costs:
                    literal_costs[node] = index;

        return literal_costs
